<?php
/**
 * Kube functions and definitions
 *
...
function kube_setup() {
	/*
		...
		* If you're building a theme based on Kube, use a find and replace
		* to change 'kube' to the name of your theme in all the template files.
		*/
	load_theme_textdomain( 'kube', get_template_directory() . '/languages' );
	...
	register_nav_menus(
		array(
			'menu-1' => esc_html__( 'Primary', 'kube' ),
		)
	);
...
add_action( 'after_setup_theme', 'kube_setup' );
 ...
 */
function kube_content_width() {
	$GLOBALS['content_width'] = apply_filters( 'kube_content_width', 640 );
}
add_action( 'after_setup_theme', 'kube_content_width', 0 );
 ...
 */
function kube_widgets_init() {
	register_sidebar(
		array(
			'name'          => esc_html__( 'Sidebar', 'kube' ),
			'id'            => 'sidebar-1',
	...
	register_sidebar(
		array(
			'name'          => esc_html__( 'Button Login', 'kube' ),
			'id'            => 'btn-login',
			'description'   => esc_html__( 'Add widgets here.', 'kube' ),
			'before_widget' => '<section id="%1$s" class="widget %2$s">',
			'after_widget'  => '</section>',
...
add_action( 'wp_enqueue_scripts', 'kube_scripts' );
...
require get_template_directory() . '/inc/class-kubet-icons.php';
